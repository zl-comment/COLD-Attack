...
from award.reaward import compute_adv_loss_from_safe_baseline_min_gap, build_safe_prompt, compute_cw_loss, \
    ...
    compute_semantic_reject_loss_steeper
from model.Apimodel import compute_fine_grained_loss
...
# loss_balancer = UncertaintyWeighting(highlight_idx=2, target_lambda=-2.0, gamma=0.5).to(device)
#
# # 在训练循环中：
# loss_total = loss_balancer(loss1, loss2, loss3, loss4, loss5)
# loss_total.backward()
# optim.step()
#
#
...
#     def __init__(self):
#         super().__init__()
#         self.log_vars = nn.Parameter(torch.zeros(5))  # 对应4个loss
#
#     def forward(self, loss1, loss2,loss3, loss4, loss5):
#         # 确保所有损失为标量
#         losses = [
#             loss1.mean() if len(loss1.shape) > 0 else loss1,
#             loss2.mean() if len(loss2.shape) > 0 else loss2,
#             loss3.mean() if len(loss2.shape) > 0 else loss3,
#             loss4.mean() if len(loss4.shape) > 0 else loss4,
#             loss5.mean() if len(loss5.shape) > 0 else loss5
#         ]
...
def decode_proxy_little(target_model_path,proxy_model, proxy_tokenizer, device, x="", z="", constraints=None, args=None, sys_prompt=None, prefix=None,
    ...
    if not args.useapi:
        ...
        print("using api",target_model_path) #此时target_model_path = api
    ...
    # 突出第3项(loss3)，让 λ₃ 靠近 -2.0，正则强度 γ=0.5
    loss_balancer = UncertaintyWeighting(highlight_idx=3, target_lambda=-2.0, gamma=0.5).to(device)
